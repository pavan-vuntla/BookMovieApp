...
const Filters = (props) => {
  ...
  const filterClickHandler = () => {
    const filterURL = `${baseUrl}movies?genre=${selectedGener.join(
      ","
    )}&title=${movieName}&artists=${selectedArtist.join(
      ","
    )}&start_date=${releaseDateStart}&end_date=${releaseDateEnd}`;
  ...
  const dateChangeHandler = (e, type) => {
    ...
    type === "startdate"
      ? setReleaseDateStart(releaseDate)
      : setReleaseDateEnd(releaseDate);
  };
    ...
    <Card>
      ...
      <CardContent>
        <FormControl className={classes.cardComponent}>
          ...
          <Input id="my-input" value={movieName} onChange={handleMovieChange} />
        </FormControl>
        <FormControl className={classes.cardComponent}>
